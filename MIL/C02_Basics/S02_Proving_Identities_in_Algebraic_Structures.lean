...
theorem neg_eq_of_add_eq_zero {a b : R} (h : a + b = 0) : -a = b := by
  rw [← add_neg_cancel_right a b, h, zero_add, neg_neg]
...
theorem neg_neg (a : R) : - -a = a := by
  nth_rewrite 1 [← zero_sub,← neg_add_cancel a, add_comm, add_sub_assoc, sub_self, add_zero]
  rfl
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  ...
  rw [← mul_assoc (a * a⁻¹), ←mul_assoc, ←mul_assoc, inv_mul_cancel, one_mul] at h
  exact h
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← one_mul (b⁻¹ * a⁻¹), ← inv_mul_cancel (a * b)]
  rw [mul_assoc, ← mul_assoc (a * b), mul_assoc a b,
    mul_inv_cancel, mul_one,
    mul_inv_cancel, mul_one]

theorem one_inv : (1 : G)⁻¹ = (1:G) := by
  rw [← one_mul (1 : G)⁻¹, mul_inv_cancel]
